		...
		async.each(helpers.regions.cloudtrail, function(region, rcb){
			...
			async.each(describeTrails.data, function(trail, cb){
				var getBucketVersioning = (cache.s3 &&
										   ...
										   cache.s3.getBucketVersioning['us-east-1'] &&
										   cache.s3.getBucketVersioning['us-east-1'][trail.S3BucketName]) ?
										   cache.s3.getBucketVersioning['us-east-1'][trail.S3BucketName] : null;
			...
			if (includeSource) {
				source = {
					getBucketVersioning: (cache.s3 && cache.s3.getBucketVersioning) ?
									      cache.s3.getBucketVersioning : null,
