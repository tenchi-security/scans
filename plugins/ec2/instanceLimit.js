...
module.exports = {
	...
	run: function(cache, includeSource, callback) {
		...
		async.each(helpers.regions.ec2, function(region, rcb){
			...
			var describeInstances = (cache.ec2 &&
									 cache.ec2.describeInstances &&
									 cache.ec2.describeInstances[region]) ?
									 cache.ec2.describeInstances[region] : null;
			...
			if (includeSource) {
				source = {
					...
					describeInstances: (cache.ec2 && cache.ec2.describeInstances) ?
									 	cache.ec2.describeInstances : null
				}
			}
